...
exports.deletePost = async (req, res) => {
  ...
  if (isNaN(parsedPostId)) {
    return res
      .status(400)
      .json({ error: "post_id และ user_id ต้องเป็นตัวเลข" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "ไม่สามารถลบโพสต์ได้", details: err.message });
  }
};
...
exports.updatePost = async (req, res) => {
  ...
  if (!post_desc) {
    return res
      .status(400)
      .json({ error: "กรุณาใส่ข้อความใหม่ของโพสต์ (post_desc)" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "ไม่สามารถแก้ไขโพสต์ได้", details: err.message });
  }
};
...
exports.getPost = async (req, res) => {
  ...
  try {
    ...
    if (search) {
      searchCondition = `
        AND (
          t2.username ILIKE '%${search}%' OR
          t1.post_desc ILIKE '%${search}%'
        )
      `;
    }
